...
# other functions
test_that("basic find_effort w/ all catch fisheries", {
  # Model info
...
test_that("find_effort w/ all effort fisheries", {
  # Model info
  season_names <- dimnames(m(bet))$season
  nseasons <- length(season_names)
  nfisheries <- dim(effort(bet_fish))[3]
  ...
  # Are we allowed 0 targets? No. Bombs
  target[target <= 0] <- 1e-9
  # Lower fishery 32 target
  target[, ac(start_year:final_year), 32] <- 50
  # Set type based on effort_ and catch_fisheries
  target_type <- rep(0, nfisheries)
  target_type[effort_fisheries] <- 1
  # Main projection loop
  ...
  expect_equal(
    ...
    round(as.numeric(target[, ac(ycount), , scount, , iter_count, drop = TRUE]), 4)
  )
})

test_that("Skip find_effort() if all effort fisheries", {
  # Model info
  season_names <- dimnames(m(bet))$season
  nseasons <- length(season_names)
  nfisheries <- dim(effort(bet_fish))[3]
