...
setMethod(
  "simpleBiol", signature(object = "FLQuant"),
  function(object, ...) {
    args <- list(...)
    # empty object
    object[] <- NA
    ...
    movement <- array(NA,
      dim = c(nareas, nareas, nages, nseasons, niters),
      dimnames = list(to = dmns$area, from = dmns$area, age = dmns$age, season = dmns$season, iter = dmns$iter)
    )
    rec_dist <- FLQuant(NA, dimnames = list(age = "all", season = dmns$season, area = dmns$area, iter = dmns$iter))
...
setMethod(
  "simpleBiol", signature(object = "missing"),
  function(...) {
    args <- list(...)
...
setMethod(
  "simpleFisheries", signature(object = "FLQuant"),
  function(object, ...) {
    args <- list(...)
    # empty object
    object[] <- NA
...
setMethod(
  "simpleFisheries", signature(object = "missing"),
  function(...) {
    args <- list(...)
    slots <- unlist(lapply(args, function(x) is(x, "FLQuant")))
    slots <- names(slots[slots])
