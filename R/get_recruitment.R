...
#' @param rec_dist An FLQuant with nseasons and nareas that splits total annual recruitment across seasons and areas.
#' @aliases get_annual_recruitment get_annual_recruitment-method
#' @examples
#' \dontrun{
#' data(bet_projection_low_catch_bits_flr)
#' annual_rec <- get_annual_recruitment(bet, year=ycount, srr_devs=srr_devs, rec_dist=rec_dist)
#' }
setGeneric("get_annual_recruitment", function(object, ...) standardGeneric("get_annual_recruitment"))

setMethod("get_annual_recruitment", signature(object="FLBiol"),
  function(object, year, srr_devs, rec_dist){
    # Check dims of srr_devs and rec_dist
    ...
    total_rec <- (ssb_total * params(object@rec)["a"]) / (ssb_total +  params(object@rec)["b"]) * exp(params(object@rec)["sigma"]/2)
